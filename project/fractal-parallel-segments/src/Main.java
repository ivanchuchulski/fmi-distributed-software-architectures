...
public class Main {
	...
	public static void main(String[] args) {
		...
		ArrayList<SegmentTask> tasks = new ArrayList<>();
		...
		for (int threadIndex = 0; threadIndex < threadCount; threadIndex++) {
			ArrayList<SegmentTask> threadTasks = new ArrayList<>();

			for (SegmentTask task : tasks) {
				if (task.id % threadCount == threadIndex) {
					threadTasks.add(task);
				}
			}

			FractalPartThread fractalPartThread = new FractalPartThread(threadIndex, bufferedImage, threadTasks);
